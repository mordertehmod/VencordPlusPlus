...
function createVencordPlusPlusMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "VencordPlusPlus",
            submenu: [
                {
                    label: "About VencordPlusPlus",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update VencordPlusPlus" : "Check for Updates",
...
export function patchTrayMenu(): void {
    ...
    Menu.buildFromTemplate = function (template: MenuItemConstructorOptions[]) {
        const alreadyPatched = template.some(item => item.label === "VencordPlusPlus");
        if (isTrayMenu(template) && !alreadyPatched) {
            const insertIndex = findInsertIndex(template);
            const vencordPlusPlusItems = createVencordPlusPlusMenuItems();
            template.splice(insertIndex, 0, ...vencordPlusPlusItems);
        }
